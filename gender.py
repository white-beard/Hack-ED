...
gendernet = load_model(params["gender_classifier_weights"])


def detect_and_predict_gender(frame, faceNet, gendernet):
    (h, w) = frame.shape[:2]
    ...
    if len(faces)>0:
        ...
        preds = gendernet.predict(faces, batch_size = 32)      
    return (locs, preds)
    ...
    def images(self):
        while True:
            ...
            if ret:    
               ...
               (locs, preds) = detect_and_predict_gender(frame, faceNet, gendernet)
               ...
               for (box, pred) in zip(locs, preds):
                    ...
                    label = "{}: {:.2f}%".format(label, max(female,male) * 100)
